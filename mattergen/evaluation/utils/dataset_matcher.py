...
def get_unique_groups(structure_matcher: StructureMatcher, structures: List[Structure]) -> List[int]:
    """Returns a list of group IDs (one per structure).
    Structures that match each other get the same group ID."""
    if len(structures) == 0:
        return []
    unique_structures: list[Structure] = []
    group_ids: list[int] = []
    for idx, structure in enumerate(structures):
        matched_group = None
        for group_id, structure_2 in enumerate(unique_structures):
            if structure_matcher.fit(structure, structure_2):
        ...
        if matched_group is not None:
            ...
            group_ids.append(len(unique_structures))
            unique_structures.append(structure)
    return group_ids
...
class OrderedDatasetUniquenessComputer:
    ...
    def compute_groups(self, dataset: ReferenceDataset) -> np.typing.NDArray[np.int_]:
        """Returns an array of unique group IDs per structure."""
        result = np.full(len(dataset), -1, dtype=int)
        global_group_counter = 0
        for reduced_formula, data_entries in dataset.entries_by_reduced_formula.items():
            structures = [e.structure for e in data_entries]
            local_groups = get_unique_groups(self.structure_matcher, structures)
            for local_idx, entry in enumerate(data_entries):
                result[entry.entry_id] = local_groups[local_idx] + global_group_counter
            global_group_counter += max(local_groups, default=-1) + 1
        return result
...
class DisorderedDatasetUniquenessComputer:
    ...
    def compute_groups(self, dataset: "ReferenceDataset") -> np.typing.NDArray[np.int_]:
        ...
        result = np.full(len(dataset), -1, dtype=int)
        global_group_counter = 0
        for chemsys, data_entries in dataset.entries_by_chemsys.items():
            structures = [e.structure for e in data_entries]
            local_groups = get_unique_groups(self.structure_matcher, structures)
            for local_idx, entry in enumerate(data_entries):
                result[entry.entry_id] = local_groups[local_idx] + global_group_counter
