...
class StructureMetricsCapability(BaseMetricsCapability):
    ...
    @cached_property
    def unique_group_ids(self) -> numpy.typing.NDArray[np.int_]:
        """Returns an array of unique group IDs per structure.
        Structures that match each other share the same group ID."""
        return self.uniqueness_computer.compute_groups(self.dataset)
